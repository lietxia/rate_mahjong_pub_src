var webobj = {
	...
	excel1: function () {
		...
		if (!!window.ActiveXObject || "ActiveXObject" in window) {
			...
			js.setAttribute('src', 'https://cdn.staticfile.net/xlsx/0.18.0/shim.min.js');
			document.body.appendChild(js);
		}
		var js2 = document.createElement('script');
		js2.setAttribute('src', 'https://cdn.staticfile.net/xlsx/0.18.0/xlsx.full.min.js');
	...
	log_page: function (full) {
		...
		for (var i = start; i < end; i++) {
			...
			var text = [
				...
				+ value[1] + '&name=' + encodeURIComponent(value[2]) + '">'
				+ value[2]
				+ '</a>', value[3], value[4] / 10,

				'<a target="_blank" href="./chart/?area='
				+ value[1] + '&name=' + encodeURIComponent(value[5]) + '">'
				+ value[5]
				+ '</a>', value[6], value[7] / 10,

				'<a target="_blank" href="./chart/?area='
				+ value[1] + '&name=' + encodeURIComponent(value[8]) + '">'
				+ value[8]
				+ '</a>', value[9], value[10] / 10,

				'<a target="_blank" href="./chart/?area='
				+ value[1] + '&name=' + encodeURIComponent(value[11]) + '">'
				+ value[11]
	...
	web_fn: {
		main: function () {
			...
			webobj.change_jumb("雀庄公式战", '线下各地雀庄，共同承认的线下段位。');
